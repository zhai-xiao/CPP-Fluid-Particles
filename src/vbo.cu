...
__global__ void generate_dots_CUDA(float3* dot, float3* posColor, float3* pos, float* density, const int num)
{
	...
	if (density[i] < 0.75f)	{
		...
		float w = (density[i] - 0.75f) * 4.0f;
		posColor[i] = w * make_float3(0.9f) + (1 - w) * make_float3(0.34, 0.46, 0.7);
	}
	else {
		float w = (powf(density[i], 2) - 1.0f)*4.0f;
		w = fminf(w, 1.0f);
		posColor[i] = (1-w)*make_float3(0.9f) + w*make_float3(1.0, 0.4, 0.7);
	}
}
